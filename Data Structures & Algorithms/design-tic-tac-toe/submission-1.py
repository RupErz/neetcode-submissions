class TicTacToe:
    ...
    # Track the count whenever user make a move
    # Only check the line relate to the marked spot
    # FIRST: understand matrix pattern (diag, antidiag, rows, cols)
    ...
    def move(self, row: int, col: int, player: int) -> int:
        ...
        if (self.rowsCount[player][row] + 1 == self.ROWS 
            or self.colsCount[player][col] + 1 == self.COLS):
            return player

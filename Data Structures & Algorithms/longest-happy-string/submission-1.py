class Solution:
    def longestDiverseString(self, a: int, b: int, c: int) -> str:
        ...
        # Pop the most freq char
        # Check last 2 if possible to see if its validate
        # If yes, then we pop a second largest to fill in
        # If no, then we just continue extending
        # Time: O(nlogk)
        # Space: O(1)
        result = []
